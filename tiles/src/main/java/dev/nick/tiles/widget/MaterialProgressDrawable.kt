...
class MaterialProgressDrawable(context: Context, private val mAnimExcutor: View) : Drawable(), Animatable {
        ...
        val animation = object : Animation() {
            public override fun applyTransformation(interpolatedTime: Float, t: Transformation) {
                if (mFinishing) {
                    ...
                    var endTrim = startingEndTrim + minArc * START_CURVE_INTERPOLATOR.getInterpolation(interpolatedTime)
    ...
    private class Ring(private val mCallback: Drawable.Callback) {
        ...
        private fun drawTriangle(c: Canvas, startAngle: Float, sweepAngle: Float, bounds: Rect) {
            if (mShowArrow) {
                ...
                c.rotate(startAngle + (if (sweepAngle < 0) 0.0f else sweepAngle) - ARROW_OFFSET_ANGLE, bounds.exactCenterX(),
                        bounds.exactCenterY())
    ...
    companion object {
        ...
        const val DEFAULT = 1
        internal val STROKE_WIDTH_LARGE = 3f
        /**
         * Layout info for the arrowhead for the large spinner in dp
         */
        internal val ARROW_WIDTH_LARGE = 12
        internal val ARROW_HEIGHT_LARGE = 6
        ...
        private val ARROW_OFFSET_ANGLE = 0f
